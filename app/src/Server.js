...
/**
 * bytepixel SFU - Server component
 *
 * @link    GitHub: https://github.com/miroslavpejic85/bytepixelsfu
 * @link    Official Live demo: https://sfu.bytepixel.com
 * @license For open source use: AGPLv3
 * @license For commercial or closed source, contact us at license.bytepixel@gmail.com or purchase directly via CodeCanyon
 * @license CodeCanyon: https://codecanyon.net/item/bytepixel-sfu-webrtc-realtime-video-conferences/40769970
...
const jwtCfg = {
    JWT_KEY: (config.jwt && config.jwt.key) || 'bytepixelsfu_jwt_secret',
...
const defaultStats = {
    enabled: true,
    src: 'https://stats.bytepixel.com/script.js',
...
function startServer() {
    ...
    app.get('/join/', async (req, res) => {
        if (Object.keys(req.query).length > 0) {
            ...
            // http://localhost:3010/join?room=test&roomPassword=0&name=bytepixelsfu&audio=1&video=1&screen=0&hide=0&notify=1
            // http://localhost:3010/join?room=test&roomPassword=0&name=bytepixelsfu&audio=1&video=1&screen=0&hide=0&notify=0&token=token
    ...
    app.get([restApi.basePath + '/meetings'], (req, res) => {
        ...
        if (!api.isAuthorized()) {
            log.debug('bytepixel get meetings - Unauthorized', {
                header: req.headers,
                body: req.body,
            });
        ...
        log.debug('bytepixel get meetings - Authorized', {
            header: req.headers,
            body: req.body,
            meetings: meetings,
        });
    ...
    app.post([restApi.basePath + '/meeting'], (req, res) => {
        ...
        const api = new ServerApi(host, authorization);
        if (!api.isAuthorized()) {
            log.debug('bytepixel get meeting - Unauthorized', {
                header: req.headers,
                body: req.body,
            });
            return res.status(403).json({ error: 'Unauthorized!' });
        ...
        // log.debug the output if all done
        log.debug('bytepixel get meeting - Authorized', {
            header: req.headers,
            body: req.body,
            meeting: meetingURL,
        });
    ...
    app.post([restApi.basePath + '/join'], (req, res) => {
        ...
        const api = new ServerApi(host, authorization);
        if (!api.isAuthorized()) {
            log.debug('bytepixel get join - Unauthorized', {
                header: req.headers,
                body: req.body,
            });
            return res.status(403).json({ error: 'Unauthorized!' });
        ...
        // log.debug the output if all done
        log.debug('bytepixel get join - Authorized', {
            header: req.headers,
            body: req.body,
            join: joinURL,
        });
    ...
    app.post([restApi.basePath + '/token'], (req, res) => {
        ...
        const api = new ServerApi(host, authorization);
        if (!api.isAuthorized()) {
            log.debug('bytepixel get token - Unauthorized', {
                header: req.headers,
                body: req.body,
            });
            return res.status(403).json({ error: 'Unauthorized!' });
        ...
        // log.debug the output if all done
        log.debug('bytepixel get token - Authorized', {
            header: req.headers,
            body: req.body,
            token: token,
        });
